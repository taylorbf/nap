...
/*  @method  toCartesian
    ...
    @param {float} [angle]
*/
bt.toCartesian = function(radius, angle){
...
*/
bt.clip = function(value, low, high) {
...
/** @method scale
    ...
    ```
*/
bt.scale = function(inNum, inMin, inMax, outMin, outMax) {
...
*/
bt.interp = function(loc,min,max) {
...
*/
bt.rcolor = function() {
  return "rgb(" + bt.random(255) + "," + bt.random(255) + "," + bt.random(255) + ")";
}



bt.VariableSpeedInterval = function(rate,func) {
	this.rate = rate
	this.on = true;
 ...
 */
bt.interval = function(rate,func) {
	var _int = new bt.VariableSpeedInterval(rate,func)
	return _int;
}
...
bt.pick = function() {
  ...
  return arguments[bt.random(arguments.length)]
 ...
 * @type {Array}
 */
bt.major = [1/1,9/8,5/4,4/3,3/2,5/3,15/8]

/**
 * returns an octave multiplier (i.e. octave(0) return 1, octave (-1) returns 0.5)
 * @param  {integer} num Octave
 */
bt.octave = function(num) {
  return Math.pow(2,num)
}



bt.getCol = function(index,limit) {
  return index%limit;
}

bt.getRow = function(index,limit) {
  return Math.floor(index/limit);
}

bt.pick = function(array) {
  return array[bt.random(array.length)];
}
...
*/
bt.ri = bt.random = function(bound1,bound2) {
  if (!bound2) {
    bound2 = bound1
    bound1 = 0
  }
...
/** @method cycle
    ...
    @param {float} [max] Upper limit of random range.
*/
bt.cycle = function(input,min,max) {
  input++;
  if (input >= max) {
    input = min;
  }
...
bt.loadScript = function (url, callback) {
...
bt.SmartMatrix = function(cols,rows) {
